...
def startup():
    # Combine multiple messages into fewer TTS calls for faster startup
    print("Initializing Jarvis...")
    speak("Initializing Jarvis. Starting all systems and checking drivers.")
    
    print("Systems loading...")
    speak("All systems activated. Now I am online.")
    ...
    speak(f"{greeting}. Currently it is {c_time}. I am Jarvis, online and ready sir. How may I help you?")
    



def wish():
    # This function is now redundant since startup() handles greeting
    pass
...
startExecution.voice_mode = False
...
class Main(QMainWindow):
    ...
    def toggleVoiceMode(self):
        ...
        if not self.voice_mode:
            ...
            self.ui.textBrowser_3.append("Text mode activated. Type commands below.")
    ...
    def sendTextCommand(self):
        ...
        if command:
            ...
            # Process the command
            if hasattr(startExecution, 'process_command'):
                startExecution.process_command(command)
            else:
                # Start JARVIS if not running
                self.startTask()
                startExecution.process_command(command)
    
    def startTask(self):
        try:
            # Try to load GIF animations
            self.ui.movie = QtGui.QMovie("JARVIS/utils/images/live_wallpaper.gif")
            self.ui.label.setMovie(self.ui.movie)
            self.ui.movie.start()
            
            self.ui.movie2 = QtGui.QMovie("JARVIS/utils/images/initiating.gif")
            self.ui.label_2.setMovie(self.ui.movie2)
            self.ui.movie2.start()
        except:
            print("Could not load GIF animations")
        ...
        self.ui.textBrowser_3.append("JARVIS Ready! Type commands or use Voice Mode.")
